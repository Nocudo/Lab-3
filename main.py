...
from numpy import array, linspace, exp, log, diag, sqrt, clip, newaxis, maximum
...
def obtener_datos(nombre_archivo, v_offset, col_t=0, col_va=1, col_vb=2):
    df: DataFrame = limpiar(
        f"{nombre_archivo}{EXTENSION}", col_t_1=col_t, col_y_1=col_va, col_y_2=col_vb)
    return df.iloc[:, col_t], df.iloc[:, col_va], df.iloc[:, col_vb] + v_offset


def filtrar_datos(x, y, v_ventana=None):
    li = None
    ls = None
...
def valores_representativos(x, y, num_valores=20, min_puntos=20):
    x_i = x.min()
    x_s = x.max()
...
def regresion_emergencial(x, y, a, c, y_std=None, resolucion_regresion=10000):
    x_fit = linspace(x.min(), x.max(), resolucion_regresion)
    arg = ((y - c) / a) + 1
    y_log = log(maximum(arg, 1e-9))
...
def regresion_no_emergencial(x, y, a, b, c, y_std=None, resolucion_regresion=1000):
    ...
    x_fit = linspace(x.min(), x.max(), resolucion_regresion)
    (a_, b_, c_), pcov = curve_fit(modelo, x, y, [
        a, b, c], y_std, True, method="lm", maxfev=10000)
...
def graficar_regresion(x, y, std, ec_fit, nombre):
    graficar_columnas(
        ...
        nom_y="Corriente (A)",
        label=["I_promedio",
               f"Ajuste emergencial: {ec_fit[0]}", f"Ajuste no emergencial: {ec_fit[1]}"],
...
def eta(a, c, q, k, temp, res, directorio_eta, offset, v_ventana):
    ...
    for nombre in nombres_archivos_eta:
        ...
        t, v_a, v_b = obtener_datos(ruta_archivo, offset)
        graficar_voltajes(t, v_a, v_b, f"Rectificador a {color_led}")
        i = (v_a - v_b) / res
        graficar_corriente(v_b, i, f"Rectificador a {color_led}")
        filt_v_b, filt_i = filtrar_datos(v_b, i, v_ventana)
        v_umbral, i_promedio, i_std = valores_representativos(filt_v_b, filt_i)
        (v_fit_e, i_fit_e), b_e, = regresion_emergencial(
            v_umbral, i_promedio, a, c, i_std)
        (v_fit_ne, i_fit_ne), (a_ne, b_ne, c_ne), (_, b_std,
                                                   _) = regresion_no_emergencial(v_umbral, i_promedio, a, b_e, c, i_std)
...
def temperatura(a, c, q, k, eta, res, directorio_temp, offset, v_ventana):
    nombres_archivos_temp = filtrar_extension(directorio_temp)
    temp_dict = {}
    for nombre in nombres_archivos_temp:
        ruta_archivo = directorio_temp / nombre
        _, v_a, v_b = obtener_datos(ruta_archivo, offset)
        i = (v_a - v_b) / res
        filt_v_b, filt_i = filtrar_datos(v_b, i, v_ventana)
        v_umbral, i_promedio, i_std = valores_representativos(filt_v_b, filt_i)
        ...
        temp_c = temp - 273.15
        color_led = nombre.split(".")[0]
        temp_dict.update({color_led: (
            (temp, temp_c, b_ne, b_std, ec_fit_ne), (v_fit_ne, i_fit_ne, color_led))})
    return temp_dict


if __name__ == "__main__":
    a = 4e-6                # I_0
    c = 9e-6                # Componente dc
    q = 1.602176634e-19     # Carga elemental del electron
    k = 1.380649e-23        # Constante de Boltzmann
    res = 217.0             # Resistencia
    offset = -0.07          # Offset para evitar corriente negativa
    voltajes_ventana = 0.9  # Voltajes para filtrar datos
    temperaturas_kelvin = {
        "26°C": 299.15,
        "40°C": 313.15,
        "90°C": 363.15,
    ...
    temps_dict = temperatura(a, c, q, k, eta_temp_amb,
                             res, directorio_temp, offset, voltajes_ventana)
    graficar_etas([temperaturas_kelvin[label_temp_ambiente]],
                  [etas_dict[label_temp_ambiente][0]],
                  [f"({label_temp_ambiente}, {eta_temp_amb:.4f})"])
    x, y, label = [], [], []
